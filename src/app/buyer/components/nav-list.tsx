...
import { BUYER_NAV, type BuyerNavEntry, type BuyerNavGroup } from "./nav";
            ...
            return (
              <li key={child.id}>
                ...
                >
                  ...
                  {child.tag ? (
                    <FeatureTag label={child.tag} isActive={isActive} />
                  ) : null}
...
export function NavList({ activeId, badges = {} }: NavListProps) {
  return (
    <ul className="flex flex-col gap-1">
      {BUYER_NAV.map((entry: BuyerNavEntry) => {
        if (entry.kind === "group") {
          return (
            <li key={entry.id}>
        ...
        return (
          <li key={entry.id}>
            ...
            >
              ...
              {entry.tag ? (
                <FeatureTag label={entry.tag} isActive={isActive} />
              ) : null}
              {badge ? <CountBadge value={badge} isActive={isActive} /> : null}
            </a>
          </li>
