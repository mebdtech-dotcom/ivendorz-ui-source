...
export function KpiCards({ items, className, ...props }: KpiCardsProps) {
  return (
    <section
      ...
      className={cn(
        "iv-kpi-grid grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3",
        className,
      )}
      {...props}
    >
      {items.map((item) => {
        const TrendIcon =
          item.trend.direction === "up" ? TrendingUp : TrendingDown;
        const trendTone =
          item.trend.direction === "up" ? "success" : "destructive";
        const directionText =
          item.trend.direction === "up" ? "Increase of" : "Decrease of";
